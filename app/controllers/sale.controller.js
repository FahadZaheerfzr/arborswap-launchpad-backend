...
exports.create = (req, res) => {
  ...
  const sale = new Sale({
    ...
    isTrending: req.body.isTrending ? req.body.isTrending : false,
  });
  ...
  Sale.findById(id)
    .then((data) => {
      if (!data) {
        return res
          .status(404)
          .send({ message: "Not found Sale with id " + id });
      }
...
// Set isTrending, but max 3 possible so we need to check if there are already 3 trending
exports.setTrending = (req, res) => {
  const id = req.params.id;
  const isTrending = req.body.isTrending;

  Sale.find({ isTrending: true })
    .then((data) => {
      if (isTrending && data.length >= 3) {
        return res.status(400).send({
          message: "Maximum 3 trending sales allowed.",
        });
      }

      Sale.findByIdAndUpdate(id, { isTrending: isTrending }, { new: true })
        .then((updatedSale) => {
          if (!updatedSale) {
            return res.status(404).send({
              message: `Sale with ID ${id} not found.`,
    ...
    .catch((err) => {
      res.status(500).send({
        message: err.message || "Error retrieving sales.",
      });
    });
};
